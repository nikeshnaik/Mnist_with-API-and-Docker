...
def run_experiment(experiment_config: Dict, save_weights: bool, gpu_ind: int, use_wandb: bool = True):
    ...
    models_module = importlib.import_module('digit_recognizer.models')
    model_class_ = getattr(models_module, experiment_config['model'])

    networks_module = importlib.import_module('digit_recognizer.networks')
...
def _parse_args():
    ...
    parser.add_argument(
        "experiment_config",
        type=str,
        help="Experimenet JSON ('{\"dataset\": \"MnistDataset\", \"model\": \"Mnist\", \"network\": \"mlp\"}'"
    )
